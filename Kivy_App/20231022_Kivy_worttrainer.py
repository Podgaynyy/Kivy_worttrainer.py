...
class WortTrainer(App):
    ...
    def build(self):
        bl = BoxLayout(orientation = 'vertical', spacing=10, padding = 25) #um in eine Zeile verschiedene Anzahl von Spalten zu haben
            #nehme ich BoxLayout als Grundwerkstoff
        bl.add_widget(Label(text="Bibliothek erstellen", font_size=40, height=50, size_hint = (1, 0.2)))  # erste Zeile bekommt

        gl = GridLayout(cols = 2, spacing = 5, height = 20, size_hint = (1, 0.3)) #erste Gridlayout wird aus zwei spalten in 2te Zeile bl passen
        gl.add_widget(Label(text = "Wort"))
        gl.add_widget(TextInput())
        gl.add_widget(Label(text="Bedeutung"))
        gl.add_widget(TextInput())

        gl2 = GridLayout(cols = 1, rows = 3, spacing = 5, height = 300, size_hint = (1, 0.5))
        gl2.add_widget(Button(text = "Speichern"))
        gl2.add_widget(Button(text="Prüfen >"))
